			...
			try {
				...
				while(y < SIZE) {
					...
					for(int x=0; x<data.length; x++) {
						map[y][x] = Integer.parseInt(data[x]);
		...
		while (true) {
			...
			if (sel == 1) {
				...
				while (true) {
					...
					System.out.println("벽1공2골3인4삭제5종료0");
					...
					if (map[y][x] != 0 && (map[y][x] == INSTALL_WALL || map[y][x] == INSTALL_BALL || map[y][x] == GOAL))
						continue;
					...
					if (move == INSTALL_WALL)
						...
						if (ballCnt != goalCnt) {
							System.out.println("골대와 볼은 쌍을 이루어야 합니다.");
							continue;
						}
						break;
					}
